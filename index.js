...
/**
 * This function will return a promise which resolves to an object with keys from header of csv and values as array
 * @param {string} filePath Path of the csv file
 * @param {string} [encoding = 'utf-8'] - Encoding of the file 
 * @returns {Object} Object containing key value pair where keys are header of csv and values are array
 */
export const readCsvAsync = async (filePath,encoding) => {
    const result = {}
    readFile(filePath,{encoding:encoding||'utf-8'},(err,data) => {
        if (err) {
            console.error(err)
            return err;
        }
